...
async def run_evaluation(
    ...
    *,
    ...
    ac_s = AnswerCorrectness(llm=llm, embeddings=None, weights=[1.0, 0.0])

    async with sem:
        try:
            passage = passages[str(int(row["passage_id"]))]
            answer_correctness = await ac_s.ascore(
...
async def main():
    c_task: Task = Task.init(
        ...
        task_name="RAGAS evaluation AnswerCorrectness",
        task_type=TaskTypes.testing,
        tags=["eval", "RAGAS", "AnswerCorrectness"],
        reuse_last_task_id=False,
    )
    c_task.set_comment("Мета оценка метрики AnswerCorrectness RAGAS (без эмбедера)")
    ...
    qa_result = qa_result[qa_result["answer_correctness"].notna()]
    logger_c.report_single_value("ok_rows", qa_result.shape[0])

    y_true = qa_result["label"].values
    y_score = qa_result["answer_correctness"].values
