...
def main(args: argparse.Namespace):
    ...
    c_task.set_comment("Обучение случайного леса")
    ...
    _ = c_task.connect(clf.get_params(), name="RandomForest")
    fitted_attrs = {
        "n_classes_": int(clf.n_classes_),  # pyright: ignore[reportArgumentType]
        "n_features_in_": int(clf.n_features_in_),
        "n_outputs_": int(clf.n_outputs_),
        "oob_score_": getattr(clf, "oob_score_", None),
    }
    _ = c_task.connect(fitted_attrs, name="RandomForest_fitted")
